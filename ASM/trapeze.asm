org 100h
section .data
    trap_msg db 0x0A,'Cuando mide el lado?',0x09,'', 0x0D, 0x0A, '$'  ; Pregunta para el cuadrado
    result_peri db 0x0A, 'el perimetro es: ',0x09,'', 0x0D, 0x0A, '$' 
    trap_mayor_msg db 0x0A, 'Cuanto la base mayor?', 0x0D, 0x0A, '$'  ; Pregunta para el cuadrado
    trap_menor_msg db 0x0A, 'Cuanto la base menor?', 0x0D, 0x0A, '$'  ; Pregunta para el cuadrado

    ;aqui va la cantidad de lados de cada figura lo necesario para calcular el perimetro
    


section .bss
    input_buffer resb 2  ; Reserva 2 bytes para la primera entrada del usuario
    second_input resb 2  ; Reserva 2 bytes para la segunda entrada, donde se elige la figura
    buffer_lado_trap resb 102 ; maxima entrada del input
    buffer_para_entero resb 8
    buffer_peque resb 2
    buffer_result resb 8


section .text
    global _start  ; Declaración global del punto de entrada

_start:
    jmp case_trap  ; Salta a la sección que pregunta si desea iniciar


case_trap:
    ; Pregunta sobre el lado del cuadrado
    mov ah, 09h
    lea dx, trap_msg
    int 21h
    mov  byte [buffer_lado_trap], 100
    mov  byte [buffer_lado_trap+1], 0
    ; Leer el lado del cuadrado
    mov ah, 0Ah
    lea dx, buffer_lado_trap
    int 21h
    jmp input_to_ax


input_to_ax: ;este paso es para un ciclo 
    xor ax, ax
    xor bx, bx    
    xor cx,cx
    mov cl, [buffer_lado_trap+1]
    lea si, [buffer_lado_trap+2]  ; Mueve fuera del bucle 
    ;call restricciones
    xor ax, ax
    xor bx, bx    
    xor cx,cx
    mov cl, [buffer_lado_trap+1]
    lea si, [buffer_lado_trap+2]  ; Mueve fuera del bucle
    jmp loop_convertidor



loop_convertidor:
    mov dx, 0                       ; Asegura que DX esté en 
    mov dl, [si]
    cmp dl, 46
    je separacion                    ; Lee el valor en DL
    sub dx, '0'                     ; Convierte de ASCII a valor numérico
    mov bx, 10                      ; Preparación para multiplicación por 10 
    add ax, dx
    cmp cx, 1
    je  calculo_area
    inc si
    mov dl, [si]
    cmp dl,46
    je separacion                    ; Añade el dígito convertido al acumulador  
    mul bx                          ; Multiplica AX por 10 (AX = AX * 10)
    loop loop_convertidor 
    jmp calculo_area 

separacion:
    inc si
    dec cx
    dec cx
    mov [buffer_para_entero], ax
    xor ax, ax
    xor dx,dx
    jmp loop_convertidor 


calculo_area: ;dice area pero es para sumas entre entradas
    xor bx, bx
    xor cx,cx
    xor dx,dx 
    mov bx, [buffer_result+4] 
    add ax, bx
    mov bx,100
    div bx
    mov [buffer_result+4],dx
    mov [buffer_peque],ax
    mov ax, [buffer_result]
    mov bx, [buffer_para_entero]
    add ax, bx
    add ax, [buffer_peque]
    mov [buffer_result],ax
    cmp di, 0
    je calculos
    cmp di, 1
    je case_base_mayor
    cmp di, 2
    je case_base_menor
    xor dx,dx
    xor bx,bx
    jmp result_msg


calculos:
    inc di
    mov ax, [buffer_result+4]
    mov bx, 2
    mul bx
    mov bx,100
    div bx
    mov [buffer_result+4],dx
    mov [buffer_peque],ax
    mov ax, [buffer_result]
    mov bx, 2
    mul bx
    add ax, [buffer_peque]
    mov [buffer_result],ax
    mov ax, [buffer_result+4]
    jmp case_base_mayor

case_base_mayor:
    add di, 1
    xor ax,ax
    xor dx,dx
    xor bx,bx
    mov ah, 09h  ; Función de MS-DOS para imprimir cadena
    lea dx, [trap_mayor_msg]  ; Carga la dirección del mensaje de consulta del lado del cuadrado en dx
    int 21h  ; Interrupción de MS-DOS para imprimir cadena
    
    mov  byte [buffer_lado_trap],100
    mov  byte [buffer_lado_trap+1],0

    ; Leer el lado del cuadrado (suponiendo una entrada numérica)
    mov ah, 0Ah  ; Función de MS-DOS para leer cadena
    lea dx, [buffer_lado_trap]  ; Carga la dirección del buffer donde se almacenará la entrada del lado del cuadrado
    int 21h  ; Interrupción de MS-DOS para leer cadena
    jmp input_to_ax

case_base_menor:
    add di, 1
    xor ax,ax
    xor dx,dx
    xor bx,bx
    mov ah, 09h  ; Función de MS-DOS para imprimir cadena
    lea dx, [trap_menor_msg]  ; Carga la dirección del mensaje de consulta del lado del cuadrado en dx
    int 21h  ; Interrupción de MS-DOS para imprimir cadena
    
    mov  byte [buffer_lado_trap],100
    mov   byte [buffer_lado_trap+1],0

    ; Leer el lado del cuadrado (suponiendo una entrada numérica)
    mov ah, 0Ah  ; Función de MS-DOS para leer cadena
    lea dx, [buffer_lado_trap]  ; Carga la dirección del buffer donde se almacenará la entrada del lado del cuadrado
    int 21h  ; Interrupción de MS-DOS para leer cadena
    jmp input_to_ax


result_msg:
    mov ah, 09h
    lea dx, result_peri
    int 21h
    xor ax,ax
    xor bx,bx
    xor dx, dx
    mov ax,[buffer_result+4]
    jmp from_ax_db 

from_ax_db:
    xor dx,dx
    mov bx, 10
    div bx
    add dx, '0'
    push dx
    inc cx
    cmp ax, 0
    jne from_ax_db
    jmp clean_to_dec 


    
clean_to_dec:
    push 46
    xor ax,ax
    mov ax, [buffer_result]
    jmp from_ax_db_dec

from_ax_db_dec:
    xor dx,dx
    mov bx, 10
    div bx
    add dx, '0'
    push dx
    inc cx
    cmp ax, 0
    jne from_ax_db_dec
    jmp imprimir_digitos

imprimir_digitos:
    mov ah, 02h
    pop dx
    int 21h
    loop imprimir_digitos
    jmp done


done:
    ; Termina el programa
    mov ah, 4Ch
    int 21h