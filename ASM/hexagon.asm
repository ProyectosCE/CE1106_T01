org 100h
section .data
    hexa_msg db 0x0A,'Cuando mide el lado?',0x09,'', 0x0D, 0x0A, '$'  ; Pregunta para el cuadrado
...
section .bss
    ...
    buffer_lado_hexa resb 100 ; maxima entrada del input
...
_start:
    jmp case_hexa  ; Salta a la sección que pregunta si desea iniciar


case_hexa:
    ; Pregunta sobre el lado del cuadrado
    mov ah, 09h  ; Función de MS-DOS para imprimir cadena
    lea dx, [hexa_msg]  ; Carga la dirección del mensaje de consulta del lado del cuadrado en dx
    int 21h  ; Interrupción de MS-DOS para imprimir cadena
    
    mov byte [buffer_lado_hexa],100
    mov byte [buffer_lado_hexa+1],0

    ; Leer el lado del cuadrado (suponiendo una entrada numérica)
    mov ah, 0Ah  ; Función de MS-DOS para leer cadena
    lea dx, [buffer_lado_hexa]  ; Carga la dirección del buffer donde se almacenará la entrada del lado del cuadrado
...
input_to_ax: ;este paso es para un ciclo 
    ...
    xor bx, bx    
    xor cx,cx
    mov cl, [buffer_lado_hexa+1]
    lea si, [buffer_lado_hexa+2]  ; Mueve fuera del bucle 
    ;call restricciones
    xor ax, ax
    xor bx, bx    
    xor cx,cx
...
restricciones: ;verifica que la entrda solo sean numeros:
    ...
    jg case_hexa
    cmp dl,0
    jl case_hexa                    ; Convierte de ASCII a valor numérico                      ; Preparación para multiplicación por 10 
